...
character :: c80tmp*80,c80tmp2*80,c200tmp*200,c1000tmp*1000,selectyn,fchfilename*200=' '
...
! 	write(*,"(a)") "15 Output current density matrix to DMNAO.txt in current folder" !Rarely used, screen it to cut down list length
	...
	if (isel==-10) then
		...
		if (isel==7) then !Visualize saved orbitals
			...
			else if (isys==2) then !There is bug in Linux version of isosur, only when nx=ny=nz can ensure the routine doesn't crash
